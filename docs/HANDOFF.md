...
| ASR | 硅基流动 SenseVoiceSmall | 同上 | c3f3a81 |
...
#    dev.env.sh 里：SILICONFLOW_API_KEY（Embed/Rerank/VLM/ASR 共用）+ DEEPSEEK_API_KEY
source dev.env.sh

# 2) 本地 OCR 服务（需要测 OCR/扫描件时）
cd ../paddleocr && uv run python server.py   # → 127.0.0.1:8000（独立 uv 项目，不在本仓库）
...
1. 写适配器类 `model/cloud-<x>.ts` 实现对应接口（`fetch` 打端点 + 一个**纯映射函数**便于单测，fail-closed）。模板：
   ...
   - `cloud-asr.ts`（multipart `/audio/transcriptions`，WAV 头解析时长）
...
| 真适配器 | `model/{paddle-ocr,cloud-embed,cloud-rerank,cloud-vlm,cloud-asr}.ts` |
...
- **ASR 云转写固有局限**：只返文本 → 单段、无说话人 diarization、timecode=[0,WAV头时长]（非 WAV→[0,0] 仅文本可检索不可精确回放）。本地 FunASR(cam++) 可补。
