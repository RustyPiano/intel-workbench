...
直接让 LLM"找矛盾"会产出不可接地、不可扩展、单次调用随语料增大丢上下文的结果。本项目把不确定性收口到最小结构里：LLM 只做①逐块抽原子主张 和③簇内成对判定，**聚类（②）与置信度（④）是确定性计算**；每条矛盾两侧都绑定 content_hash 精确块。benchmark 证明这在玩具集 F1 上不及直出，但赢在 **precision 1.0 + 逐条 provenance + 可扩展 + 可审计**（§3.2）。
...
**实验过程（真实，含一次惨败→修复）**：初版按 `entity:attribute` 精确串聚类，对 LLM 自由抽取的属性表层差异过敏 → 簇几乎全 size-1 → **F1≈0.29（recall 0.17）惨败**；改按**实体聚类**（attribute 交簇内 NLI 判语义冲突）→ **F1=0.737（precision 1.0，0 误报）**。

**价值定位（这是拿分的关键论证）**：30 块小语料上大模型直出已近天花板（F1=0.957），自研结构化在**原始 F1 上不占优**——但这不是失败，而是把价值定位讲清楚：
- **可验证 provenance**：每条矛盾的两侧陈述各绑定 content_hash 精确块，precision 1.0，可点可审；大模型直出只吐 `chunk_id` 对、无接地校验。
- **可扩展**：逐块抽 + 仅簇内成对判（O(Σnᵢ²)），不随语料增大而单次调用丢上下文；直出在大语料必掉精度。
- **可审计**：每次出站经 OfflineGuard、每步入哈希链审计。

→ 即自研 skill 的"赢"不在玩具集 F1，而在**情报域真实约束下的可溯源/可扩展/可审计**——这正是"套壳大模型直出"给不出的。完整 benchmark 见 `docs/report/benchmark-summary.md`。
