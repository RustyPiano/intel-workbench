...
import type { AcknowledgeContradictionInput, ContradictionService } from "../analysis/contradiction-service.js";
...
export function createContradictionsRouter(contradictions: ContradictionService): Router {
  ...
  router.patch("/:id/contradictions/:contradictionId/acknowledge", async (req, res) => {
    const acknowledgement = await contradictions.acknowledge(
      req.identity,
      req.params.id,
      req.params.contradictionId,
      (req.body ?? {}) as AcknowledgeContradictionInput,
    );
